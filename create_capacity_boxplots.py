...
"""
create_capacity_boxplots.py

Creates box plots showing tour lengths (Total_Cost) vs Vehicle Capacity
for Combinatorial Auction CVRP results across 100 instances.

Reads CSV files from CA_results folder with naming pattern:
timings_capacity_100.csv, timings_capacity_120.csv, etc.
"""
...
def load_all_capacity_results(results_dir="CA_results"):
    """
    Load all capacity result CSV files from the directory.

    Expected CSV format:
    Dataset,Computation_Time_Seconds,Status,Total_Cost,Feasible_Subsets,Total_Subsets,N_Subsets_Used
    """

    # Find all CSV files matching the pattern
    csv_files = glob.glob(os.path.join(results_dir, "timings_capacity_*.csv"))
    ...
    for csv_file in sorted(csv_files):
        ...
        # Extract number from filename like "timings_capacity_100.csv"
        try:
        ...
        try:
            ...
            # Filter for successful/optimal solutions
            df_optimal = df[df['Status'] == 'Optimal']

            if len(df_optimal) > 0:
                capacity_data[capacity] = df_optimal['Total_Cost'].values
                print(f"Capacity {capacity}: {len(df_optimal)} optimal solutions, "
                      f"cost range [{df_optimal['Total_Cost'].min():.0f}, {df_optimal['Total_Cost'].max():.0f}]")
            else:
                print(f"Warning: No optimal solutions found in {basename}")
...
def create_boxplot(capacity_data, technique_name="Combinatorial Auction", 
                   output_file="CA_tourlengths_boxplot.png"):
    """
    Create box plot comparing tour lengths across different capacities.

    Args:
        capacity_data: Dictionary mapping capacity -> array of total costs
...
def main():
    ...
    print("="*70)
    print("COMBINATORIAL AUCTION CVRP - BOX PLOT GENERATOR")
    print("="*70)

    # Configuration
    results_dir = "CA_results"
    technique_name = "Combinatorial Auction"
    output_file = "CA_tourlengths_boxplot.png"
    ...
    if capacity_data:
        ...
        print(f"2. CSV files are named: timings_capacity_100.csv, timings_capacity_120.csv, etc.")
        print(f"3. CSV files have columns: Dataset, Status, Total_Cost")
